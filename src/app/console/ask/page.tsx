...
import { BrainCircuit, CirclePlay, ArrowRight, SearchCode, Gauge, Database, Sparkles, FileSearch, Loader2, CircleCheck } from "lucide-react";
import PostmortemReport, { PostmortemData } from "@/components/PostmortemReport";

export default function AskTelemetryPage() {
  ...
  const [suggestions, setSuggestions] = useState<string[]>([
    "Loading dynamic suggestions from Prometheus..."
  ]);

  useEffect(() => {
    const fetchTopServices = async () => {
      try {
        const res = await fetch('/api/ai/suggestions?window=last%202%20hours');
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      const prompt = `Based on the query plan for ${planData?.service || 'the service'}, the user asked: "${query}". 
Here is the strict, raw JSON data returned from actually executing those queries against Prometheus and Loki:
${JSON.stringify(rawResults)}

CRITICAL INSTRUCTION: You MUST NOT invent or hallucinate any probable causes, components, timelines, or fake incidents. Build the postmortem strictly and purely based on the facts present in the raw data above. Do not invent data. If the data only shows generic exceptions, state only that. Only actual causes if known from the data should be mentioned.
IMPORTANT: If the raw data contains a large time-series matrix array (e.g. many data points), DO NOT list every single data point in the timeline. Summarize the overall trend (e.g. "CPU fluctuated between 60% and 70%") in the executive summary, and only add 1 or 2 key timeline events if there was a major spike or drop. The graph will be rendered automatically by the UI.`;
      ...
      setTimeout(() => {
        document.getElementById("ask-report")?.scrollIntoView({ behavior: 'smooth' });
      }, 100);
  ...
  return (
    <div>
      <div className="mb-8 grid gap-6 lg:grid-cols-[.82fr_1.18fr] lg:items-end">
        <div>
          <div className="mb-3 text-xs font-black uppercase tracking-[.22em] text-blue-600">Ask Your Telemetry</div>
          <h2 className="text-xl font-black tracking-[-.035em] sm:text-3xl text-slate-950">Investigate systems using plain English.</h2>
        </div>
        <p className="max-w-2xl text-slate-600">
          AiSignal translates operational questions into Prometheus, Loki and Tempo queries, executes the query plan, and turns the results into a grounded investigation report.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
        <div className="border-t border-slate-200 pt-7">
          <div className="mb-4 flex items-center gap-2 text-sm font-black uppercase tracking-[.16em] text-slate-500">
            <BrainCircuit className="text-blue-600 w-4 h-4" /> Natural-language request
          </div>
          <label className="sr-only" htmlFor="telemetry-question">Telemetry question</label>
          <textarea 
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mr-1">Quick Tags:</span>
            {[
              { tag: '/trend', tooltip: 'Force a time-series graph' },
              { tag: '/average', tooltip: 'Force a scalar average' },
              { tag: '/errors', tooltip: 'Filter by 500 errors' }
            ].map(({ tag, tooltip }) => (
              <button
                key={tag}
                onClick={() => setQuery(prev => prev ? `${prev} ${tag}` : tag)}
                title={tooltip}
                className="rounded-md bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600 transition-colors hover:bg-slate-200 hover:text-slate-900"
              >
                {tag}
              </button>
            ))}
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            >
              ...
              Execute & Generate Report <ArrowRight className="w-4 h-4" />
            </button>
          </div>
          
          <div className="mt-9 border-t border-slate-200 pt-6">
            <div className="mb-4 text-xs font-black uppercase tracking-[.18em] text-slate-400">Try examples</div>
        ...
        <div className="min-w-0 border-t border-slate-200 pt-7">
          <div className="mb-5 flex items-center gap-2 text-sm font-black uppercase tracking-[.16em] text-slate-500">
            <SearchCode className="text-blue-600 w-4 h-4" /> AI-generated query plan
          </div>
          ...
          {!planData && !isLoading && (
             <div className="text-slate-500 text-sm font-semibold mt-8 flex items-center justify-center h-40 border border-dashed border-slate-300 rounded-2xl">
               Hit "Generate query plan" to see AiSignal reasoning.
             </div>
          )}
          ...
          {isLoading && !planData && (
             <div className="text-blue-600 text-sm font-bold flex flex-col items-center justify-center h-40 border border-dashed border-blue-200 bg-blue-50/50 rounded-2xl gap-3">
               <Loader2 className="w-6 h-6 animate-spin" /> Translating natural language to PromQL / LogQL...
             </div>
          )}
            ...
            <div className="space-y-7 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {isExecuting && !reportData && (
                 <div className="mt-8 text-blue-600 text-sm font-bold flex flex-col items-center justify-center h-40 border border-dashed border-blue-200 bg-blue-50/50 rounded-2xl gap-3">
                   <Loader2 className="w-6 h-6 animate-spin" /> Executing queries & analyzing results...
                 </div>
              )}
